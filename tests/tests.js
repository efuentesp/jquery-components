...
QUnit.test("QUnit DATE -- FORMAT <dd-mm-yyyy> -- DATE <31-12-2019>", function(
  assert
) {
  ...
  try {
    ...
    throw "\n\n <<" +
      inptFecha.val() +
      ">> IS NOT VALID.  FORMAT MUST BE <<dd-mm-yyyy>> AND THE DATE VALUE MUST BE VALID FOR THE CALENDAR \n\n";
  }
});

QUnit.test("QUnit DATE -- FORMAT <dd-mm-yyyy> -- DATE <12-13-2019>", function(
  assert
) {
  var formatDate = "dd-mm-yy";
...
QUnit.test("QUnit DATE -- FORMAT <dd-mm-yyyy> -- DATE <12-13-20BC>", function(
  assert
) {
  var formatDate = "dd-mm-yy";
...
var QUnitDate = QUnit.test(
  "--- QUnit DATE -- FORMAT <dd-mm-yyyy> -- DATE <12-13-19>",
  function(assert) {
      ...
      QUnit.test(
        ...
        function(assert) {
          try {
            var testdate = $.datepicker.parseDate(formatDate, inptFecha.val());
            console.log(
              "---->> testdate: " + date + " -- testdate: " + testdate
            );

            var dateee = $("#inpt-fecha")
...
QUnit.test("Datepicker seleccionando una fecha con el calendario", function(
  assert
) {
  // arrange
  const txtInputFecha = $("#inpt-fecha");
